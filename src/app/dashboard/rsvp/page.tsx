...
// ── TextField — defined outside page so hooks are at top level ─────────────
function TextField({
  label, desc, field, value, multiline = false, rows = 2, onSave,
}: {
  label: string; desc?: string; field: string; value: string
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'flex-start' }}>
          {multiline
            ? <textarea value={val} onChange={e => setVal(e.target.value)} rows={rows}
                style={{ flex: 1, padding: '10px 12px', borderRadius: 8, border: '1px solid #2a3829', fontSize: 13, background: '#141c13', color: 'var(--title)', outline: 'none', resize: 'vertical', fontFamily: 'inherit' }} />
          ...
          <button onClick={save}
            style={{ flexShrink: 0, padding: '10px 14px', borderRadius: 8, background: saved ? '#22c55e' : 'var(--accent)', color: '#fff', border: 'none', fontSize: 12, fontWeight: 700, cursor: 'pointer', transition: 'background 0.2s' }}>
            {saving ? '…' : saved ? '✓' : 'Save'}
          </button>
        </div>
      </div>
...
// ── PhotoField — defined outside page so hooks are at top level ────────────
function PhotoField({
  field, label, desc, value, saving, saved, uploading, onSave, onUpload,
}: {
  field: string; label: string; desc: string; value: string
  saving: string | null; saved: string | null; uploading: string | null
  onSave: (field: string, val: string) => Promise<void>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <input value={urlInput} onChange={e => setUrlInput(e.target.value)}
            ...
            onKeyDown={e => { if (e.key === 'Enter' && urlInput.trim()) { onSave(field, urlInput.trim()); setUrlInput('') } }}
          ...
          <button onClick={() => { if (urlInput.trim()) { onSave(field, urlInput.trim()); setUrlInput('') } }}
      ...
      <Card style={{ marginBottom: 24 }}>
        <div className="flex gap-8 items-start flex-wrap">
          <div className="flex flex-col items-center gap-3">
            <canvas ref={canvasRef} width={160} height={160} className="rounded-xl block"
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 24, marginBottom: 24 }}>
        ...
        <div>
          ...
          <p style={{ fontSize: 12, color: 'var(--body)', marginTop: 8 }}>
            Dress code color swatches are managed in <strong style={{ color: 'var(--sage)' }}>Settings → Dress code colors</strong>
          </p>
        </div>

        <div>
          <SectionLabel>Our story</SectionLabel>
          <TextField label="Story text" field="ourStory" value={s.ourStory} multiline rows={8}
        ...
        <div>
          ...
          <Card style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
            {PHOTO_FIELDS.map(({ field, label, desc, value }) => (
              <PhotoField key={field} field={field} label={label} desc={desc} value={value}
                saving={saving} saved={saved} uploading={uploading}
                onSave={saveField} onUpload={handleUpload} />
            ))}
          </Card>
        </div>
