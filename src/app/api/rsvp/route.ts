...
export async function GET(req: NextRequest) {
  ...
  // Only search invitees — exclude auto-created plus-one records
  ...
  if (attending && plus_one_name) {
    ...
    if (!existing) {
      await prisma.guest.create({
        data: { name: plus_one_name, dietary: plus_one_dietary || null, side: guest.side, hasPlusOne: false, rsvpStatus: 'attending', isInvitee: false },
      })
    }
  }
      ...
      html: `<div style="font-family:Georgia,serif;max-width:560px;margin:40px auto;background:#fff;border-radius:16px;overflow:hidden;border:1px solid #e7e2da">
        ...
        <div style="padding:32px">
          ...
          <div style="text-align:center;margin:28px 0">
            <a href="${appUrl}/rsvp" style="display:inline-block;background:#7A9C6E;color:#fff;text-decoration:none;padding:14px 32px;border-radius:50px;font-size:14px">View your RSVP ✨</a>
          </div>
        </div>
