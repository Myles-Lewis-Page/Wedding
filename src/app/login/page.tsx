    ...
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24, background: 'linear-gradient(160deg, #0d1a0c 0%, #111714 60%)' }}>
      <div style={{ width: '100%', maxWidth: 420 }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: 40 }}>
          <Heart size={24} fill="#8fb882" style={{ color: '#8fb882', margin: '0 auto 16px' }} />
          <h1 style={{ fontFamily: 'var(--font-display)', fontSize: 42, fontWeight: 300, color: '#e8f0e6', marginBottom: 6 }}>
            Jennifer & Myles
          </h1>
        ...
        <div style={{ background: '#1a2419', borderRadius: 20, padding: 36, border: '1px solid #2a3829', boxShadow: '0 30px 80px rgba(0,0,0,0.5)' }}>
          ...
          <button
            ...
            style={{ width: '100%', padding: '14px', borderRadius: 12, background: loading || !password.trim() ? '#2a3829' : '#4a7a44', color: '#e8f0e6', border: 'none', fontSize: 15, fontWeight: 600, cursor: loading || !password.trim() ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, transition: 'all 0.2s' }}
          >
            {loading ? <><Loader2 size={17} style={{ animation: 'spin 1s linear infinite' }} />Signing in…</> : 'Sign in'}
          </button>
        </div>

        <p style={{ textAlign: 'center', fontSize: 13, color: '#2a3828', marginTop: 24 }}>
